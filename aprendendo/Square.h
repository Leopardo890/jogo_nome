...
typedef struct {																												//Definição da estrutura de um quadrado (!)
	...
	unsigned char side_y;																											//Tamanmho da lateral de um quadrado (!)
...
square* square_create(unsigned char side_x, unsigned char side_y, unsigned short x, unsigned short y, unsigned short max_x, unsigned short max_y);		//Protótipo da função de criação de um quadrado (!)
