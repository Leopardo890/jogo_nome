...
bullet* pistol_shot_boss(tela *tela, unsigned short x, unsigned short y, unsigned char trajectory, pistol *gun){			//Implementação da função "pistol_shot" (!)
	
	bullet *new_bullet = bullet_create_cam(tela, x, y, trajectory, gun->shots);									//Cria uma nova instância de projétil a ser disparado (!)
...
bullet* pistol_shot_fire(tela *tela, unsigned short x, unsigned short y, unsigned char trajectory, pistol *gun){			//Implementação da função "pistol_shot" (!)
	
	bullet *new_bullet = bullet_create_fogo(tela, x, y, trajectory, gun->shots);									//Cria uma nova instância de projétil a ser disparado (!)
	if (!new_bullet) return NULL;																		//Verifica o sucesso da alocação de memória; retorna NULL em caso de falha (!)
	return new_bullet;																					//Retorna uma nova instância de projétil (!)
}

bullet* pistol_shot_k(tela *tela, unsigned short x, unsigned short y, unsigned char trajectory, pistol *gun){			//Implementação da função "pistol_shot" (!)
	
	bullet *new_bullet = bullet_create_muni(tela, x, y, trajectory, gun->shots);									//Cria uma nova instância de projétil a ser disparado (!)
	if (!new_bullet) return NULL;																		//Verifica o sucesso da alocação de memória; retorna NULL em caso de falha (!)
	return new_bullet;																					//Retorna uma nova instância de projétil (!)
}
